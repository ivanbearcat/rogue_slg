@tool
class_name FxMagicAuraEffect
extends FxParticleEffect


## FxMagicAuraEffect — 魔法光环 环境特效（持续型）。
## 实例化 magic_aura.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/magic_aura.tscn")
