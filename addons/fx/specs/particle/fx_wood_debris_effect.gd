@tool
class_name FxWoodDebrisEffect
extends FxParticleEffect


## FxWoodDebrisEffect — 木屑 环境特效（持续型）。
## 实例化 wood_debris.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/wood_debris.tscn")
