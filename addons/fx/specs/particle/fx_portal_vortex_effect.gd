@tool
class_name FxPortalVortexEffect
extends FxParticleEffect


## FxPortalVortexEffect — 传送门漩涡 环境特效（持续型）。
## 实例化 portal_vortex.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/portal_vortex.tscn")
