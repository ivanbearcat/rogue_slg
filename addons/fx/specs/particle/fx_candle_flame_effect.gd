@tool
class_name FxCandleFlameEffect
extends FxParticleEffect


## FxCandleFlameEffect — 烛光 环境特效（持续型）。
## 实例化 candle_flame.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/candle_flame.tscn")
