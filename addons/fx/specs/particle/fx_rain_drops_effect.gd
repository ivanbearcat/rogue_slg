@tool
class_name FxRainDropsEffect
extends FxParticleEffect


## FxRainDropsEffect — 雨滴 环境特效（持续型）。
## 实例化 rain_drops.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/rain_drops.tscn")
