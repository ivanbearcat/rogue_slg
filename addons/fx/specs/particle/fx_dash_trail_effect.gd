@tool
class_name FxDashTrailEffect
extends FxParticleEffect


## FxDashTrailEffect — 冲刺拖尾特效（持续型）。
## 实例化 dash_trail.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/dash_trail.tscn")
