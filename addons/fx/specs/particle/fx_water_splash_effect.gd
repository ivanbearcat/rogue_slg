@tool
class_name FxWaterSplashEffect
extends FxParticleEffect


## FxWaterSplashEffect — 水花溅射 环境特效（持续型）。
## 实例化 water_splash.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/water_splash.tscn")
