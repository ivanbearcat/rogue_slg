@tool
class_name FxCampfireSmokeEffect
extends FxParticleEffect


## FxCampfireSmokeEffect — 篝火烟雾 环境特效（持续型）。
## 实例化 campfire_smoke.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/campfire_smoke.tscn")
