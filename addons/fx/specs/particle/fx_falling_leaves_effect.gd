@tool
class_name FxFallingLeavesEffect
extends FxParticleEffect


## FxFallingLeavesEffect — 落叶 环境特效（持续型）。
## 实例化 falling_leaves.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/falling_leaves.tscn")
