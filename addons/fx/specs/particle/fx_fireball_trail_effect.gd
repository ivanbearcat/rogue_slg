@tool
class_name FxFireballTrailEffect
extends FxParticleEffect


## FxFireballTrailEffect — 火球拖尾 环境特效（持续型）。
## 实例化 fireball_trail.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/fireball_trail.tscn")
