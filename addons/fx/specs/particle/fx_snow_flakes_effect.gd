@tool
class_name FxSnowFlakesEffect
extends FxParticleEffect


## FxSnowFlakesEffect — 雪花 环境特效（持续型）。
## 实例化 snow_flakes.tscn 粒子场景到目标位置。
## 返回节点不自动 free，由调用方管理。


@export var scene: PackedScene = preload("res://addons/fx/effects/snow_flakes.tscn")
