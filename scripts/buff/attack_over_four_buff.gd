...
func process_buff():
	if Current.slime_die_sum >= 4:
		Current.public_lock_array.append("attack_over_four_buff")
		var add_num = int(Current.once_total_score * 0.2)
		var float_number_instantiate = EffectManager.float_number_effect(add_num)
		Current.hero.add_child(float_number_instantiate)
		EffectManager.big_flow_effect(buff_texture)
		await Tools.time_sleep(1.5)
		Current.total_score += add_num
		Current.public_lock_array.erase("attack_over_four_buff")
